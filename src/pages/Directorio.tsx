...
export const Directorio: React.FC = () => {
  ...
  const getBuildInfo = () => {
    ...
    const buildDate = new Date('2025-09-22T19:47:55.054Z'); // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Actualizado automáticamente // Fecha actualizada automáticamente
...
Esta acción no se puede deshacer y eliminará todas las etapas asociadas.`
  ...
  const handleSaveEdit = async (index: number, title: string, description: string, duration: number, alertColor: string, alertSeconds: number) => {
    try {
      ...
      if (index === currentStageIndex && !isTimerRunning) {
        ...
        sendControlCommand('setTime', { seconds: duration });
    ...
    if (!meetingWindow || meetingWindow.closed) {
      ...
      if (newMeetingWindow) {
        ...
        if (!window.syncService) {
          window.syncService = createSyncService();
          window.syncService.setDirectoryId(selectedMeeting?.id || 'default');
          window.syncService.setIsMainWindow(true);
          window.syncService.startHeartbeat();
          
          // Configurar callbacks
          window.syncService.onConnection((state) => {
            setConnectionState(state);
            addConnectionLog(`Estado de sincronización: ${state.connected ? 'Conectado' : 'Desconectado'}`);
          });
          
          window.syncService.onMessage((message: SyncMessage) => {
  ...
  const sendControlCommand = (action: string, data?: Record<string, any>): void => {
    ...
    // También enviar directamente a la ventana si está abierta
    if (meetingWindow && !meetingWindow.closed) {
      try {
  ...
  const sendTimerState = (): void => {
    ...
    // Enviar estado a través del servicio de sincronización
    if (window.syncService) {
      window.syncService.sendTimerState(timerState);
    }
  };
  ...
  const handlePreviousStage = () => {
    if (currentStageIndex > 0) {
      ...
      setIsTimerRunning(false);
      sendControlCommand('previousStage', { stageIndex: newIndex });
  ...
  const handleNextStage = () => {
    if (currentStageIndex < stages.length - 1) {
      ...
      // Pausar el cronómetro al cambiar de etapa
      setIsTimerRunning(false);
      sendControlCommand('nextStage', { stageIndex: newIndex });
  ...
  useEffect(() => {
    if (selectedMeeting) {
      addConnectionLog('Sistema de sincronización local activado');
  ...
  useEffect(() => {
    ...
    const confirmStop = window.confirm(
     ...
     setTimerUpdate(prev => prev + 1);
            ...
            if (initialTimeValue) {
              sendControlCommand('setTime', { seconds: parseInt(initialTimeValue) });
            }
  ...
  // Función para forzar reconexión del sistema de sincronización
  const forceControlReconnection = () => {
    console.log('🔄 Forzando reconexión del sistema de sincronización...');
    addConnectionLog('Reconexión manual solicitada');
    
    // Reconectar servicio de sincronización si está disponible
    if (window.syncService) {
      window.syncService.disconnect();
      setTimeout(() => {
        window.syncService = createSyncService();
        window.syncService.setDirectoryId(selectedMeeting?.id || 'default');
        window.syncService.setIsMainWindow(true);
        window.syncService.startHeartbeat();
        addConnectionLog('Sistema de sincronización reconectado');
      }, 1000);
    }
  };
              ...
              <div className="text-sm text-gray-600">
               ...
               <div className="mt-4 flex space-x-2">
                 ...
                 <button
                   ...
                   title="Forzar reconexión del sistema de sincronización"
                 >
                   🔄 Reconectar
                 </button>
               </div>
